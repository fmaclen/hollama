...
test('model select supports searching and filtering', async ({ page }) => {
	await page.goto('/settings');
	const modelComboBox = page.getByLabel('Available models');
	await modelComboBox.click();
	...
	await modelComboBox.fill('gemma');
	await expect(page.getByText('gemma:7b')).toBeVisible();
	await expect(page.getByText('TheBloke/CodeLlama-70B-Python-AWQ')).not.toBeVisible();
	await expect(page.getByText('openhermes2.5-mistral:latest')).not.toBeVisible();

	await modelComboBox.fill('');
	await expect(page.getByText('gemma:7b')).toBeVisible();
	await expect(page.getByText('TheBloke/CodeLlama-70B-Python-AWQ')).toBeVisible();
	await expect(page.getByText('openhermes2.5-mistral:latest')).toBeVisible();

	await modelComboBox.fill('nonexistentmodel');
	await expect(page.getByText('No results')).toBeVisible();
});

test('models are correctly grouped including recently used models', async ({ page }) => {
	await page.goto('/settings');
	await mockTagsResponse(page);
	...
	const recentlyUsedModel = recentlyUsedModelsGroup.locator('.field-combobox-item-label-option', {
		hasText: MOCK_API_TAGS_RESPONSE.models[1].name
	});
	await expect(recentlyUsedModel).toBeVisible();

	const otherModelsGroup = page.locator('.field-combobox-group', {
		hasText: 'Other models'
	});
	await expect(otherModelsGroup).toContainText(MOCK_API_TAGS_RESPONSE.models[0].name);
	await expect(otherModelsGroup).toContainText(MOCK_API_TAGS_RESPONSE.models[2].name);
});

test('models are correctly sorted by last used', async ({ page }) => {
	await page.goto('/settings');
	await mockTagsResponse(page);
